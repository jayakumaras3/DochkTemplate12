...
async function fetchData() {
	try {
		...
		const quizData = getRandomQuestionsAndAnswers(answers, questions);
...
function getRandomQuestionsAndAnswers(answers, questions) {
	...
	questionRandom = questions.QuestionRandom;
	optionRandom = questions.OptionRandom;
	totalQuestions = questions.TotalQuestions;
	
	QuizAttempt=questions.QuizAttempt;
	Resulttitle=questions.Resulttitle;
	resutscorecontent=questions.Resutscorecontent;
	Resutpassed=questions.Resutpassed;
	Resutfailed=questions.Resutfailed;
	Questiontext=questions.Questiontext;
	
	duration = parseInt(questions.duration) * 60;
	duration1 = parseInt(questions.duration);
	startpagedescrip= questions.startpagedescrip;
	resultpagedescrip= questions.resultpagedescrip;
	startpageheader=questions.startpageheader;
	startpagedescrip1= questions.startpagedescrip1;
	startButton= questions.startButton;
	quizButton= questions.quizButton;
	//viewResult= questions.viewResult;
	clicknote= questions.clicknote;
	TotalQuestionste=questions.TotalQuestionste;
	passingScorete=questions.passingScorete;
	QuizAttemptte=questions.QuizAttemptte;
	durationte=questions.durationte;
	parent.setPassScore(questions.passingScore);
	retryButton=questions.retryButton
	TimeUpte=questions.TimeUpte;
	...
	if (questionRandom == 1) {
		...
		// Ensure no duplicates by removing selected questions from the pool
	/*	while (randomQuestions.length < Math.min(totalQuestions, questionPool.length)) {
			const randIndex = Math.floor(Math.random() * questionPool.length);
			console.log(randIndex);
			randomQuestions.push(questionPool[randIndex]); // Add the random question
			randomAnswers.push(answerPool[randIndex]); // Add the corresponding answer
		//	console.log(randIndex);
			// Remove the selected question and answer to prevent duplicates
 ...
 //  console.log("Shuffled Answer Data:", JSON.stringify(shuffledAnswerData, null, 2));
	...
	PassingScore = questions.passingScore;

	if (questions.iframeSrc) {
		// Create iframe element
		const iframe = document.createElement('iframe');
		iframe.id = 'contentFrame';
		iframe.src = questions.iframeSrc;
