    ...
    if(scorm.connection.isActive){
        ...
        if(API){
	 		...
	 		if(scorm.handleExitMode && !exitStatus){
				...
				//------------ ORGcalculateelapsedTime Updated By Gopi.S------------------------------ ---------------------
				var success = false; 
				var dtm = new Date(); 
				var n = (dtm.getTime() - g_dtmInitialized.getTime())/1000; 
				elapsedTime=ORGformatelapsedTime(n);
				//alert("elapsedTime : "+elapsedTime);
				switch(scorm.version){
					//case "1.2" : success =  scorm.set("cmi.core.session_time","0010:34:34.56");
					case "1.2" : success =  scorm.set("cmi.core.session_time",elapsedTime);
					break;
					case "2004": success = scorm.set("cmi.session_time",elapsedTime); 
					break;
				}									
				//-----------------------------------------------------------------------------------------
    ...
    if(scorm.connection.isActive){
        ...
        if(API){
			...
			switch(scorm.version){
			 ...
			 //API.LMSSetValue("cmi.core.session_time", "0010:34:34.56")
            if(success){
...
// --------------- Updated by Gopi.S -------------------------------------------
function ORGformatelapsedTime(totalSecondsinau)
{
	/*	
	...
	*/
	...
	return totalHours + ":" + totalMinutes + ":" + totalSeconds+".00";
...
function SCOReportSessionTime() { 
    ...
    var dtm = new Date(); 
    var n = (dtm.getTime() - g_dtmInitialized.getTime()) / 1000; 
    var elapsedTime = ORGformatelapsedTime(n);

    var scorm = pipwerks.SCORM; // FIX: define scorm here
    console.log("Session time to report: " + elapsedTime + " for SCORM version " + scorm.version);

    switch(scorm.version) { 
        case "1.2":
            success = scorm.set("cmi.core.session_time", elapsedTime);
            break; 
        case "2004":
            success = scorm.set("cmi.session_time", elapsedTime);
            break; 
    }
