(function() {
	...
	p.setTocConfig = function(response) {
		if (response.data.toc == undefined) {
			//this.globalVariableService.toclevel=false
			...
			for (var y = 0; y < Object.keys(response.data).length; y++) {
				...
				/* this.tocContent += "<input class='toggle-box' id='header" + y + "' type='checkbox' onchange='changingcheckbox(" + y + ")'><label for='header" + y + "'  data-toggle='collapse' data-target='#demo" + y + "' class='sideBartopicHeader'>" + this.globalVariableService.maintopicarray[y] + "</label> ";
				 this.tocContent += '<ol id="tocData"  id="demo' + y + '" class="collapse" >';*/

				for (var i = 0; i < this.looptocData.length; i++) {
					countpage++;
					Totalpage = countpage;
				//	console.log("countpage:: " + Totalpage);
					...
					if (data != undefined) {
						...
						//console.log("i"+st);
						//console.log("i"+st1);
						//console.log("i"+AudioVersionEnable);
						if (AudioVersionEnable) {
							//console.log("audio"+st2);
							if (i == 1) {
								//console.log("i"+st2);
								this.tocContent += '<span style="display: flex;"><li style="display:none;" class="disabledClass" id' + st + ' ><span id' + st1 + ' role="button" onclick="tocDataClick(this)"  > ' + data + '</span> </li></span>';
							} else {
								...
								/*		this.tocContent += '<li class="disabledClass" id' + st + ' ><span id' + st1 + ' role="button" onclick="tocDataClick(this)"  > ' + data + '</span> <span class="tickSymbol"> </span></li>'+
									'<p class="tickSymbol"></p>' +
								'</span>';*/
		...
		switch (name) {
			case 'toc':
				...
				setTimeout(function() {
					...
					// page tiltle enable when click transcript and menu toggle start
					//console.log("TotalPageNo2:: "+Totalpage)
					pageSormTrack();
	...
	p.changeTocData = function() {
		...
		if (this.globalVariableService.toclevel == false) {
			...
			var transcriptData = '<img class="transcriptDownL" role="button" title="Learning Aids" ng-controller="sideBar as sb" id="mute" ' +
				'onclick="pdfLoader()" ' +
				'src="theme/images/footer-menu/Setting.png" alt="image">' +
