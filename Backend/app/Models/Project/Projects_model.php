...
class Projects_model extends Model
{
    ...
    protected $allowedFields = ['projectname', 'theme', 'reviewpopup', 'status', 'createdby', 'createdon'];
    ...
    function removeassetfolder($assethistoryid)
    {
        ...
        $builder->set('last_updated_by', session()->get('username'));
    ...
    function displayclosedproject()
    {
        ...
        $user = session()->get('username');
        $builder = $this->db->table('projects as p');
        $builder->select('p.*,p.projectid as projectsid,c.course_id,count(distinct c.course_id) as course_count,d.name as theme,pd.*,max(pd.accesslevel),p.createdby as add_by,p.createdon as add_dt,p.last_updated_on as del_dt,p.status as stat,p.completion,cl.client_name,u.name as fullname');
        $builder->join('dropdown as d', 'd.id_d = p.project_type', 'left');
        $builder->join('users as u', 'u.username = p.createdby', 'left');
        $builder->join('projects_details as pd', 'pd.projectid = p.projectid ', 'left');
        $builder->join('courses as c', 'c.project_id = p.projectid and c.status!=0', 'left');
    ...
    function importnewpage($courseid, $sheetData)
    {
        for ($i = 0; $i < count($sheetData); $i++) {
            ...
            $q = " INSERT INTO feedback (`feedbackid`, `course_id`, `pageid`, `feedback`, `type`, `stage`, `status`, `private`, `comment_type`, `comment_category`, `serverity`, `createdby`, `createdon`, `videotime`) 
