...
class Settings_model extends Model
{
    ...
    public function get_client_status(int $cid): array
    {
        $builder = $this->db->table('project_ucn as ucn');
        $builder->select('ucn.remarks, ucn.ucn_id, ucn.status, ucn.name, ucn.start_dt, ucn.end_dt, ucn.scope, ucn.last_updated_on');
        $builder->where('ucn.status !=', 10);
        $builder->where('ucn.status !=', 0);
        $builder->where('ucn.client', $cid);
        $builder->orderBy('ucn.ucn_id', 'ASC');
        $data = $builder->get()->getResultArray();
        return $data;
    }

    public function menu_view(int $cid): array
    {
    ...
    public function marketplaceaccess(int $cid): array
    {
