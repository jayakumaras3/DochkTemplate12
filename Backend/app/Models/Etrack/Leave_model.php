...
class Leave_model extends Model
{
    ...
    function in_office_today_details($today)
    {
        ...
        $builder->select('guser.id_user, guser.name, guser.last_name, guser.profile_image, guser.profile_foldername, SUM(ep.num_days) as numday');
    ...
    function getLeaveBalanceByUser($user, $start_year, $end_year)
    {
        $builder = $this->db->table('et_leave as lev');
        $builder->select("lev.type,
            SUM(CASE WHEN lev.number_leave > 0 THEN lev.number_leave ELSE 0 END) AS allotted,
            SUM(lev.number_leave) AS balance");
        $builder->where('lev.emp_id', $user);
        $builder->where('lev.status !=', 0);
        $builder->where('lev.start_dt >=', $start_year);
        $builder->where('lev.start_dt <=', $end_year);
        $builder->groupBy('lev.type');
        $data = $builder->get()->getResultArray();
        return $data;
    }
