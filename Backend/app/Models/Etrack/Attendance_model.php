...
class Attendance_model extends Model
{
    ...
    function getleaveformonth($user, $start_date, $end_date)
    {
        ...
        $builder->select('ANY_VALUE(etl.start_dt) as start_dt, SUM(etl.number_leave) as number_leave, ANY_VALUE(etl.type) as leave_type');
    ...
    function getAccessCardDataLite($id_user, $start_date, $end_date)
    {
        ...
        $total = 0;
        foreach ($data as $row) {
            $actualMinutes = $this->actualhrToMinutes($row['actualhr']);
            if ($actualMinutes >= 450) { // 07:30
                $total += 1;
            } elseif ($actualMinutes > 210) { // 03:30
                $total += 0.5;
            }
        }
        $data[] = ['total' => $total];
    ...
    private function actualhrToMinutes($actualhr)
    {
        if (empty($actualhr)) {
            return 0;
        }
        $parts = explode(':', $actualhr);
    ...
    function getWfhById($et_wfh_id)
    {
        $builder = $this->db->table('et_workfromhome as wfh');
        $builder->select('wfh.*');
        $builder->where('wfh.et_wfh_id', $et_wfh_id);
        $data = $builder->get()->getResultArray();
        return $data[0] ?? null;
    }

    function sumWfhForPeriod($user, $start_date, $end_date)
    {
        $builder = $this->db->table('et_workfromhome as wfh');
        $builder->select('SUM(wfh.number_wfh) AS total_wfh');
        $builder->where('wfh.emp_id', $user);
        $builder->where('wfh.start_date >=', $start_date);
        $builder->where('wfh.start_date <=', $end_date);
        $builder->where('wfh.status', 1);
        $data = $builder->get()->getResultArray();
        return (float) ($data[0]['total_wfh'] ?? 0);
    }

    function getRecentWfh($user, $limit = 5)
    {
        $builder = $this->db->table('et_workfromhome as wfh');
        $builder->select('wfh.*');
        $builder->where('wfh.emp_id', $user);
        $builder->where('wfh.status', 1);
        $builder->orderBy('wfh.start_date', 'DESC');
        $builder->orderBy('wfh.et_wfh_id', 'DESC');
        $builder->limit($limit);
    ...
    public function getattendanceMonthlyReport($startDate, $endDate, $userId = null)
    {
        ...
        if ($userId !== '1' && $userId !== '1115' && $userId !== '834') {
