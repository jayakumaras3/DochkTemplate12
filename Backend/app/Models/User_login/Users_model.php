...
class Users_model extends Model
{
    ...
    {
        $builder = $this->db->table('users as u');
        $builder->select('u.*,du.*,dc.name  as category_name,d.name as  Category_item,d.createdon');
        $builder->join('dropdown_users as du', 'du.fk_id_user = u.id_user ', "left");
        $builder->join('dropdown_category as dc', 'dc.id_dc = du.fk_id_dc and du.fk_id_dc =2 ', "left");
    ...
    public function countUsersByStatus($client_id, $valid)
    {
        $builder = $this->db->table('users as u');
        $builder->where('u.client_id', $client_id);
        $builder->where('u.valid', $valid);
        return $builder->countAllResults();
    }

    public function countUsersByRole($client_id, $access_id)
    {
        $builder = $this->db->table('dropdown_users as du');
        $builder->join('users as u', 'u.id_user = du.fk_id_user', 'left');
        $builder->where('u.client_id', $client_id);
    ...
    public function getUserclientlist_onlyname_etrack_dashbaord($client_id)
    {
        $builder = $this->db->table('users as u');
        $builder->where('u.client_id', $client_id);
        $builder->where('u.valid', '1');
