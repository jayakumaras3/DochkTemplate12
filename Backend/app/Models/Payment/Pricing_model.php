...
class Pricing_model extends Model
{
    ...
    public function addPrice($newdata)
    {
        // print_r($newdata);
        // exit();
        $db = \Config\Database::connect();
        ...
        if ($insertID) {
            ...
            //     'createdon' => time(),
            // ];
            // $data = $this->updateHisPartnerDisdetails($newhisdata);
            return  $data;
        }
    }
    public function getPriceDetails()
    {
        $builder = $this->db->table('price as pr');
        $builder->select('pr.*');
        $builder->where('pr.status', '1');
        $data = $builder->get()->getResultArray();
        return  $data;
    }
    public function geteachPriceDetails($pr_id)
    {
        $builder = $this->db->table('price as pr');
        $builder->select('pr.*');
        $builder->where('pr.pr_id', $pr_id);
        $builder->where('pr.status', '1');
        $data = $builder->get()->getResultArray();
        return  $data;
    }
    public function editpricedetails($newdata, $pr_id){
        $builder = $this->db->table('price as pr');
        $builder->where('pr_id', $pr_id);
        $builder->update($newdata);
        $data = $builder->get()->getResultArray();
        return  $data;
    }
