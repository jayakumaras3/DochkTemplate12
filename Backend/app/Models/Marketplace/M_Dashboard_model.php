...
class M_Dashboard_model extends Model
{
    ...
    // Shared join/filter builder for the PHP-rendered catalog page (marketplace_view). Unlike
    // get_courses(), $category is optional here so "All" genuinely means no category filter,
    // instead of relying on a magic "All" category row in scorm_meta_category.
    private function catalogBaseQuery($client_id, $mp_language, $category, $search)
    {
        $builder = $this->db->table('marketplace_clients as mc');
        $builder->join('marketplace as m', 'm.mp_id =  mc.mp_id AND m.status != 0', 'left');
        $builder->join('marketplace_courses as mpc', 'mpc.mp_id =  mc.mp_id AND mpc.status != 0', 'left');
        ...
        $builder->where('mc.client_id', $client_id);
        $builder->where('m.type', 1);
        // scorm_courses is a left join so a marketplace_courses row whose scorm_id no longer
        // matches an active course (deleted/deactivated) would otherwise still surface as a
        // blank tile (empty name/language/duration, no valid scourse_id to link to).
        $builder->where('c.scourse_id IS NOT NULL', null, false);

        if (!empty($mp_language) && $mp_language != 'All') {
            $builder->where('c.language', $mp_language);
        }
    ...
    function getCatalogCourseCount($client_id, $mp_language, $category, $search = '')
    {
        $builder = $this->catalogBaseQuery($client_id, $mp_language, $category, $search);
        $builder->select('COUNT(DISTINCT mpc.mp_co_id) as total');
        $row = $builder->get()->getRowArray();
        return (int) ($row['total'] ?? 0);
    }

    function getCatalogCourses($client_id, $mp_language, $category, $search, $sortColumn, $sortDir, $limit, $offset)
    {
        $builder = $this->catalogBaseQuery($client_id, $mp_language, $category, $search);
        $builder->select('mc.discount, mc.billing_cycle, mc.payment_type, mc.currency, mc.cost, mpc.price, mpc.scorm_id, mpc.mp_co_id,
                  c.scourse_id, c.course_name, c.course_code, c.duration,
        ...
        if (isset($sortMap[$sortColumn])) {
            $builder->orderBy($sortMap[$sortColumn], $sortDir);
        } else {
            $builder->orderBy('mpc.mp_co_id', 'DESC');
        }
    ...
    // Sidebar language counts react to the selected category only (search/language selection
    // don't affect them), matching how the catalog UI recalculates counts on category change.
    function getCatalogLanguageCounts($client_id, $category)
    {
        $builder = $this->catalogBaseQuery($client_id, 'All', $category, '');
        $builder->select('c.language, COUNT(DISTINCT c.scourse_id) as langcount');
        $builder->groupBy('c.language');
        return $builder->get()->getResultArray();
    }
    ...
    function get_courses_by_id($mp_id)
    {
        ...
        $builder->join('page as p', 'p.fk_course_id = c.scourse_id AND  p.type = 4 and  p.status != 0', 'left');
        $builder->join('assessment_questions as aq', 'aq.page_id = p.page_id AND aq.status != 0', 'left');
        $builder->join('assessment_settings as aset', 'aset.scourse_id = c.scourse_id AND aset.status != 0 and aset.type = 23', 'left');
    ...
    {
        $builder = $this->db->table('marketplace as mp');
        $builder->select('mp.*, u.name as owner_name');
 ...
 (SELECT COUNT(mc.client_id) 
    ...
    function certification_learning_plan_courses($certificate_id, $client, $user_id, $type)
    {
        $builder = $this->db->table('certificates as cert');
        // $builder = $this->db->table('certificates_assigned_users as cert_users');
        $builder->select(' mp.mp_id, mp.mp_name,mp.duration,mp.thumbnail,mp.description,mp.mode,cert.name as cert_name,sc.scourse_id,sc.course_name,sc.course_code,sc.duration as course_duration,sc.language,sc.avg_rating,sc.thumbnail as course_thumbnail,aulp.status as learning_plan_status,pm.amount');
        $builder->join('certificates_assigned as cert_assign', 'cert_assign.certificate_id = cert.cert_id', 'left');
        $builder->join('assign_users_learning_plan as aulp', 'aulp.mp_id = cert_assign.course_lp_mp_id AND aulp.user_id = ' . $user_id . ' AND aulp.status != 0', 'left');
        $builder->join('marketplace as mp', 'mp.mp_id = cert_assign.course_lp_mp_id', 'left');
        ...
        $builder->join('certificates_assigned_users as cert_users', 'cert_users.certificate_id = cert.cert_id ', 'left');
        ...
        $builder->where('cert.cert_id', $certificate_id);
        $builder->where('cert_assign.type', 4);
        $builder->where('mp.status !=', 0);
        $data = $builder->get()->getResultArray();
        // echo $this->db->getLastQuery();
