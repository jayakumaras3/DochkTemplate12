...
class Scorm_course_model extends Model
{
    ...
    protected $allowedFields = ['course_name', 'description', 'thumbnail', 'status', 'createdby', 'createdon', 'last_updated_by', 'last_updated_on', 'project_id', 'type', 'course_code', 'duration', 'language', 'avg_rating'];
    ...
    //     return  $data;
    // }
    // --- Paginated/server-side variants for the SCORM/scorm_courses DataTable ---
    // coursenamessearch_view20()/201() below fetch every matching course unbounded (no LIMIT) and
    // are shared by several other pages (Gamification, My_training, Scorm_learn_group) that
    // genuinely need the full list for dropdowns, so their behavior can't change. These new
    // methods back the courses_datatable_ajax() endpoint instead, doing the paging/search/sort
    // in SQL so a page load only ever touches one page's worth of rows, not all ~1600.

    private function coursesSearchApply($builder, $search)
    {
        if ($search === '') {
            return;
        }
        $builder->groupStart();
        $builder->like('c.course_name', $search);
        $builder->orLike('c.course_code', $search);
        $builder->orLike('c.language', $search);
        $builder->groupEnd();
    }

    private function coursesAdminBaseQuery($client_id, $id_user)
    {
        $builder = $this->db->table('scorm_courses_assigned as sc');
        $builder->join('scorm_users_courses_assigned as sca', 'sca.course_id = sc.course_id and sca.id_user = ' . (int) $id_user . ' and sca.status = 1 and sca.type = 0', 'left');
        ...
        $builder->join('users as u', 'u.id_user = c.createdby', 'left');
        $builder->where('sc.client_id', $client_id);
        $builder->where('sc.status', 1);
        $builder->where('c.status', '1');
        return $builder;
    }

    private function coursesUserBaseQuery($id_user)
    {
    ...
    private function coursesSortMap()
    {
        return [
    ...
    // The Status column badge (buildCourseDatatableRow() in Scorm_courses.php) isn't a plain DB
    // column - it's derived from mode (draft states take priority), then course_status, then
    // lesson_status. This CASE expression mirrors that exact priority so "sorting" the column
    // groups rows the same way they'd visually bucket: drafts, Completed, In Progress, Not Started.
    private function courseStatusSortExpr()
    {
        return "CASE
            WHEN c.mode IN (1,3,4,5,6,7,8) THEN 0
            WHEN MAX(sca.course_status) = 2 THEN 1
            WHEN MAX(sca.course_status) = 1 OR MAX(su.lesson_status) IN ('incomplete','failed') THEN 2
            ELSE 3
        END";
    }

    function getCoursesAdminDatatableCount($client_id, $id_user, $search = '')
    {
        $builder = $this->coursesAdminBaseQuery($client_id, $id_user);
        $this->coursesSearchApply($builder, $search);
        $builder->select('COUNT(DISTINCT sc.course_id) as total');
        $row = $builder->get()->getRowArray();
        return (int) ($row['total'] ?? 0);
    }

    function getCoursesAdminDatatable($client_id, $id_user, $search, $orderColumn, $orderDir, $limit, $offset)
    {
        $builder = $this->coursesAdminBaseQuery($client_id, $id_user);
        $this->coursesSearchApply($builder, $search);
        $builder->select('c.course_name as course_name, c.course_code, c.duration as duration, c.scourse_id as scourse_id,
                  c.demo as demo, c.language, u.name as createdby, c.createdon,
                  MAX(su.lesson_status) as lesson_status, MAX(sca.last_updated_on) as last_updated_on,
                  c.promo_video as promo_video, c.thumbnail, c.type, c.mode, MAX(sca.course_status) as course_status');
        $builder->groupBy('sc.course_id');

        $sortMap = $this->coursesSortMap();
        if ($orderColumn === 'status') {
            $builder->orderBy($this->courseStatusSortExpr(), $orderDir, false);
        } elseif (isset($sortMap[$orderColumn])) {
            $builder->orderBy($sortMap[$orderColumn], $orderDir);
        } else {
            $builder->orderBy('c.course_name', 'ASC');
        }
    ...
    function getCoursesUserDatatableCount($id_user, $search = '')
    {
        $builder = $this->coursesUserBaseQuery($id_user);
        $this->coursesSearchApply($builder, $search);
        $builder->select('COUNT(DISTINCT sca.course_id) as total');
        $row = $builder->get()->getRowArray();
        return (int) ($row['total'] ?? 0);
    }

    function getCoursesUserDatatable($id_user, $search, $orderColumn, $orderDir, $limit, $offset)
    {
        $builder = $this->coursesUserBaseQuery($id_user);
        $this->coursesSearchApply($builder, $search);
        $builder->select('c.course_name as course_name, c.course_code, c.type, c.mode, c.duration as duration,
                  c.scourse_id as scourse_id, c.demo as demo, c.language, u.name as createdby, c.createdon,
                  MAX(su.lesson_status) as lesson_status, MAX(sca.last_updated_on) as last_updated_on,
                  c.promo_video as promo_video, c.thumbnail, MAX(sca.course_status) as course_status');
    ...
    public function coursenamessearch_view201()
    {
        ...
        $builder->select('c.course_name as course_name,c.course_code,c.type,c.mode,c.duration as duration,c.scourse_id as scourse_id,c.demo as demo,c.language,u.name as createdby,c.createdon,MAX(su.lesson_status) as lesson_status, MAX(sca.last_updated_on) as last_updated_on,c.promo_video as promo_video,c.thumbnail,MAX(sca.course_status) as course_status');
        $builder->join('scorm_courses as c', 'c.scourse_id = sca.course_id', 'left');
        $builder->join('scorm_user_details as su', 'su.user_assign_id = sca.user_assign_id and su.status =1', 'left');
        $builder->join('users as u', 'u.id_user = c.createdby', 'left');
    ...
    public function coursenamessearch_view20()
    {
        ...
        $builder = $this->db->table('scorm_courses_assigned as sc');
        $builder->select('c.course_name as course_name,c.course_code,c.duration as duration,c.scourse_id as scourse_id,c.demo as demo,c.language,u.name as createdby,c.createdon,MAX(su.lesson_status) as lesson_status, MAX(sca.last_updated_on) as last_updated_on,c.promo_video as promo_video,c.thumbnail,c.type,c.mode,MAX(sca.course_status) as course_status');
    ...
    public function getEnrolledLearnersCount($scourse_id)
    {
        $builder = $this->db->table('scorm_users_courses_assigned');
        $builder->select('COUNT(DISTINCT id_user) as cnt');
        $builder->where('course_id', $scourse_id);
