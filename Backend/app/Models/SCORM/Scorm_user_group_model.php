...
class Scorm_user_group_model extends Model
{
    ...
    function getUsergroupdata($type, $client)
    {
        $db = \Config\Database::connect();
        ...
        $builder->join('scorm_user_group_assigned as ac1', 'ac1.group_id = mc.sc_cgid and ac1.status!=0', 'left');
        $builder->groupby('mc.sc_cgid');
        $builder->where('scgc.status =', '1');
        $builder->where('scgc.client_id =', $client);
        $builder->where('ac1.status =', '1');
        $data = $builder->get()->getResultArray();
        return $data;
    }
