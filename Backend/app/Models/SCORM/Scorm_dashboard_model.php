...
class Scorm_dashboard_model extends Model
{
    ...
    public function getCoursesAssignedToMeSummary($id_user)
    {
        // Lean variant of getCoursesAssignedToMe() for the dashboard summary tiles/carousels,
        // which never display "category" - skips the assign_meta_category/scorm_meta_category
        // joins and GROUP_CONCAT so the query has fewer joins and no per-row string aggregation.
        $builder = $this->db->table('scorm_users_courses_assigned as sca');
    ...
    public function isassignCourseToUser($user, $courseId)
    {
        return $this->db
    ...
    public function getCoursesdetails($course_id)
    {
        ...
        $builder = $this->db->table('scorm_users_courses_assigned as sca');
        $builder->select('c.*,MAX(su.lesson_status) as lesson_status,Max(su.attempt) as attempt,MAX(sca.course_status) as course_status,MAX(sca.due_date) as due_date,MAX(u.name) as createdby,MAX(ub.name) as last_updated_by_name');
        $builder->join('scorm_courses as c', 'c.scourse_id = sca.course_id', 'left');
        ...
        $builder->join('users as ub', 'ub.id_user = c.last_updated_by', 'left');
        $builder->where('sca.id_user', $id_user);
        $builder->where('sca.course_id', $course_id);
