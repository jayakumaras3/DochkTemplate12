...
class Scorm_learn_group_model extends Model
{
    ...
    {
        // group_id is NOT NULL on scorm_course_group_assigned; bail out instead of letting
        // an empty group_id reach the insert and surface as a raw DB error.
        if (empty($newdata['group_id']) || empty($newdata['course_id'])) {
            return false;
        }
        foreach ($newdata['course_id'] as $course_id) {
            $scorm_course_group_assigned = [
                'course_id' => $course_id,
                'group_id' => isset($newdata['group_id']) ? $newdata['group_id'] : null,
    ...
    function getCoursesAssignedto($sc_cgid)
    {
        ...
        $db = \Config\Database::connect();
        $builder = $this->db->table('scorm_course_group_assigned as mc');
        $builder->select('ANY_VALUE(mc.assign_id) as assign_id,sc_co.*,sc_co.course_name as coursename,sc_co.scourse_id as scourse_id,MAX(su.lesson_status) as lesson_status,
    MAX(sca.course_status) as course_status');
        $builder->join('scorm_courses as sc_co', 'sc_co.scourse_id = mc.course_id', 'left');
        $builder->join('scorm_users_courses_assigned as sca', 'sca.course_id = sc_co.scourse_id and sca.id_user =' . session()->get("id_user"), 'left');
