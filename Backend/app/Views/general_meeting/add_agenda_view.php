...
<div class="row">
    <div class="col-md-6">
        <div class="card">
            <div class="card-body">
                <form class="form" action="<?php echo base_url('general_meeting_agenda/addagenda_header') ?>" method="POST"><?= csrf_field() ?>
                    <div class="mb-3">
                ...
                foreach ($meetingagendaheader as $eachMeetingAgendaHeader) {
                    ...
                    <tr>
                        ...
                        <?php if ($user == $eachMeetingAgendaHeader['createdby']) { ?>
                            <td><a href="<?php echo base_url('general_meeting_agenda/copyagenda_header?projectid=' . $eachMeetingAgendaHeader['fk_project_id'] . '&id_m=' . $eachMeetingAgendaHeader['id_m']); ?>"><button type="submit" class="btn btn-sm btn-info"><span class="icon-copy"></span></button></a></td>
                            <td>
                                <form action="<?php echo base_url('general_meeting_agenda/editagenda_header') ?>" method="POST"><?= csrf_field() ?>
                                    <input type="hidden" name="id_m" value="<?php echo $eachMeetingAgendaHeader['id_m'] ?>">
                                    <button type="submit" class="btn btn-sm btn-warning">
                                        <span class="icon-pencil"></span></button>
                                </form>
                            </td>
                            <!-- <a href="<?php echo base_url('general_meeting_agenda/editagenda_header?projectid=' . $eachMeetingAgendaHeader['fk_project_id'] . '&id_m=' . $eachMeetingAgendaHeader['id_m']); ?>"><button type="submit" class="widget-icon btn-warning"><span class="icon-pencil"></span></button></a> -->
                            </td>
                            <td><a onclick="return confirm('<?php echo lang('Alert.Aler_002') ?>')" href="<?php echo base_url('general_meeting_agenda/delagenda_header?projectid=' . $eachMeetingAgendaHeader['fk_project_id'] . '&id_m=' . $eachMeetingAgendaHeader['id_m']); ?>"><button type="submit" class="btn btn-sm btn-danger"><span class="icon-trash"></span></button></a></td>
                        <?php } ?>
                    </tr>
