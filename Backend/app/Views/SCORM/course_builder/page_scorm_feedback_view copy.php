...
<ul class="nav nav-tabs">
    <li class="<?php if ($tab == 1) echo "active"; ?>"><a data-toggle="tab" href="#newfeedback" <?php if ($tab == 1) echo "active"; ?>>New</a></li>
    <li class="<?php if ($tab == 2) echo "active"; ?>"><a data-toggle="tab" href="#myfeedback" <?php if ($tab == 2) echo "active"; ?>>My</a></li>
    <li class="<?php if ($tab == 3) echo "active"; ?>"><a data-toggle="tab" href="#allfeedback" <?php if ($tab == 3) echo "active"; ?>>All</a></li>
    <li class="<?php if ($tab == 4) echo "active"; ?>"><a data-toggle="tab" href="#closedfeedback" <?php if ($tab == 4) echo "active"; ?>>Closed</a></li>
</ul>
<style>
    .custom-row {
        display: flex;
...
<div style="margin: 15px" ;>
    ...
    <div class="tab-content">
        <div id="newfeedback" class="tab-pane fade <?php if ($tab == 1) echo "in active"; ?>">
            <div class="feedback_form">
                <form class="feedback_inside_form" method="POST" action="<?php echo base_url('SCORM/Course_builder/review_course/addNewfeedback') ?>" enctype="multipart/form-data" id="addNewfeedback" method="POST"><?= csrf_field() ?>
                    <div class="row">
                        <div class="col-md-12">
        ...
        <div id="myfeedback" class="tab-pane fade <?php if ($tab == 2) echo "in active"; ?>">
            <?php $j = 0;
                ...
                if ($user_id == $feedback_details['createdby'] && $feedback_details['status'] != 6) {
                    ...
                    <div class="individual_feedback_design" style="position: relative; width: 100%;">
                        <?php if (!empty($feedback_details['profile_image'])  &&  !empty($feedback_details['profile_foldername']) ) { ?>
                            <img class="img_circle" src="<?php echo (base_url() . 'assets/assets/uploads/profile/' . $feedback_details['id_user'] . '/' . $feedback_details['profile_foldername'] . '/' . $feedback_details['profile_image']) ?>" class="rounded-circle" alt="Profile" width="50" height="50">
                        <?php } ?>
                        ...
                        <div class="dropdown" style="float:right;">
                            ...
                            <div class="dropdown-content">
                                <form class="delete_feedback" method="POST"><?= csrf_field() ?>
                                    ...
                                    <input type="hidden" name="feedbackid" value="<?php echo $feedback_details['feedbackid']; ?>" />
                                    <button type="submit" class="btn btn-sm btn-danger" style="width: 100%; padding: 10px 5px; border: none;text-align: center; cursor: pointer;">
                                        Delete
                                    </button>
                                </form>
                        ...
                        <?php if (strlen($feedback_details['uploaded_file'] ?? '') > 2) {
                            ...
                            <div class="feedback_inside_form" style="float:right;"> <button class="btn btn-outline-warning  btn-xs rounded-pill waves-effect waves-light" data-toggle="modal" data-target="#myModal2" title="Upload">
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <div class="form-row">
                                            <p style="font-size:10px">File type : png,jpg and jpeg </p>
                                            <form class="form-horizontal" enctype="multipart/form-data" id="uploadmyfile" method="POST"><?= csrf_field() ?>
                        ...
                        <?php if (isset($replies[$feedback_details['feedbackid']])) { ?>
                            <?php foreach ($replies[$feedback_details['feedbackid']] as $reply) { ?>
                                <?php if (!empty($feedback_details['profile_image']) && !empty($feedback_details['profile_foldername'])) { ?>
                                    <img class="img_circle_reply" src="<?php echo (base_url() . 'assets/assets/uploads/profile/' . $reply['id_user'] . '/' . $reply['profile_foldername'] . '/' . $reply['profile_image']) ?>" class="rounded-circle" alt="Profile" width="50" height="50">
                                <?php } ?>
                        ...
                        <div class="reply_form">
                            <form class="replay_inside_form" id="replyfeedback" method="POST"><?= csrf_field() ?>
                                <table style="width:100%">
                                    <tr>
                                        <td style=" padding: 2px;"><textarea name="feedback" class="form-control" placeholder="Reply" rows="1" cols="30" required></textarea></td>
                                        <td style=" padding: 2px;"><input type="hidden" name="course_id" value="<?php echo $course_id; ?>" />
                                            <input type="hidden" name="page_id" value="<?php echo $page_id; ?>" />
                                            <input type="hidden" name="feedbackid" value="<?php echo $feedback_details['feedbackid']; ?>" />
                                            <input type="hidden" name="status" value="2" />
                                            <input type="hidden" id="tab" name="tab" value="2" />
                                            <button type="submit" class="btn btn-sm btn-default"><i class="fa fa-reply" style="color: grey; font-size: 15px;"></i></button>
                                        </td>
                                    </tr>
                                </table>
        ...
        <div id="allfeedback" class="tab-pane fade <?php if ($tab == 3) echo "in active"; ?>">
        ...
        <div id="closedfeedback" class="tab-pane fade <?php if ($tab == 4) echo "in active"; ?>">
            <?php $j = 0;
            $user_id = session()->get('id_user');
            foreach ($feedbacks as $feedback_details) {
                if ($feedback_details['createdby'] == $user_id && $feedback_details['status'] == 6) {
                    ...
                    <div class="individual_feedback_design" style="position: relative; width: 100%;">
                        ...
                        <div class="feedback_inside_form" style="float:right;"> <button class="btn btn-outline-warning  btn-xs rounded-pill waves-effect waves-light" data-toggle="modal" data-target="#myModal">
                                <i class="fa fa-paperclip"></i>
                            </button>
                        </div>
                        <?php if (strlen($feedback_details['uploaded_file'] ?? '') > 2) { ?>
                        ...
                        <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <div class="form-row">
                                            <form class="form-horizontal" enctype="multipart/form-data" id="uploadfile" method="POST"><?= csrf_field() ?>
                                                <div class="form-group col-md-12 mb-3">
                                                    <input type="file" name="file" required />
                                                </div>
                                                <div class="form-group col-md-12">
...
<script>
    function capture() {
        ...
        alert(<?php echo $row['course_type']; ?>);
        if (window.self === window.top) {
            console.log("Running in the PARENT page.");
        } else {
            console.log("Running inside the IFRAME (child pagxxxe).");
        }
       
        // var elmnt = null;
        <?php if ($row['course_type'] == '10') { ?>
            console.log("Course type 10.");
            const iframe = window.parent.document.getElementById("target");
            if (iframe) {
  // The div element exists
  console.log('Div exists!');
} else {
  // The div element does not exist
