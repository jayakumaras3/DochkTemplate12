...
class Certification_dashboard extends BaseController
{
    ...
    public function index()
    {
        ...
        echo view('certification/certification_maindashboard_view', $data);
        echo view('templates/footer_view', $data);
    }
    public function downloadCertificationsJson()
    {
        if ($response = $this->requireRole(['5', '44', '3'])) {
            return $response;
        }
        ...
        $result = $this->Certification_model->json_all_certifications($client);

        if (empty($result)) {
            return $this->response
                ->setStatusCode(404)
                ->setBody('No certifications found.');
        }
        ...
        foreach ($result as $row) {
            ...
            if (!isset($certificates[$certId])) {
                $certificates[$certId] = [
                    ...
                    'duration' => $row['certificate_duration'],
                    'learning_paths' => []
                ];
            }

            if (!isset($certificates[$certId]['learning_paths'][$lpId])) {
                $certificates[$certId]['learning_paths'][$lpId] = [
                    'lp_id' => $lpId,
                    'lp_name' => $row['lp_name'],
            ...
            if (!empty($row['scourse_id'])) {
                $certificates[$certId]['learning_paths'][$lpId]['courses'][$row['scourse_id']] = [
                    'course_id' => $row['scourse_id'],
        ...
        $jsonData = array_values($certificates);

        $filename = 'certifications_' . date('Ymd_His') . '.json';
    ...
    public function downloadCertJson()
    {
        if ($response = $this->requireRole(['5', '44', '3'])) {
            return $response;
        }
        ...
        $client = session()->get('client');
        $summary = $this->Certification_model->summary($client);
        $details = $this->Certification_model->details($client);
        $final = [];

        foreach ($summary as $cert) {

            $certId = $cert['certificate_id'];
            $certDuration = $cert['certificate_duration'] ?? '0'; // Default to '0' if not set

            $duration = $certDuration;

            if ($duration >= 60) {
                $hours = intdiv($duration, 60);
                $balancemin = $duration % 60;

                $formattedDuration = $hours . ' Hrs';

                if ($balancemin > 0) {
                    $formattedDuration .= ' ' . $balancemin . ' Mins';
                }
            } else {
            ...
            $final[$certId] = [
                ...
                "price" => '₹ ' . number_format(
                    $price,
                    ($price == floor($price)) ? 0 : 2
                ),

                "duration" => $formattedDuration, // keep your logic here

                "total_courses" => $cert['total_courses'],
                "total_learning_paths" => $cert['total_learning_paths'],
        ...
        foreach ($details as $row) {
            ...
            if (!isset($final[$certId]['learning_paths'][$lpId])) {

                $final[$certId]['learning_paths'][$lpId] = [
                    "lp_id" => $lpId,
                    "lp_name" => $row['lp_name'],
                    "lp_description" => $row['lp_description'],
                    "lp_banner" => base_url('assets/assets/img/default_learning_plan_banner.jpg'),
                    "courses" => []
                ];
            }

            if (!empty($row['scourse_id'])) {
                $final[$certId]['learning_paths'][$lpId]['courses'][$row['scourse_id']] = [
                    "course_id" => $row['scourse_id'],
                    "course_name" => $row['course_name']
                ];
            }
        }
        foreach ($final as &$cert) {
            $cert['learning_paths'] = array_values($cert['learning_paths']);

            foreach ($cert['learning_paths'] as &$lp) {
                $lp['courses'] = array_values($lp['courses']);
            }
        }


        $jsonData =  array_values($final);
        $filename = 'certifications_' . date('Ymd_His') . '.json';
    ...
    public function add_new_certificate()
    {
        ...
        return redirect()->to(base_url() . 'Certification/Certification_dashboard');
    }
    ...
    public function Edit_certificate()
    {
        ...
        $data['getassignedclients'] = $this->Certification_model->getassignedclients($data['certificate_id']);
        echo view('templates/header_view', $data);
        echo view('certification/certification_edit_view', $data);
        echo view('templates/footer_view', $data);
    }
    function assign_certification_to_client()
    {
        ...
        $editable = $this->request->getPost('editable');
        $data_client = [
        ...
        return redirect()->to(base_url() . 'Certification/Certification_dashboard/Edit_certificate');
    }
    function delete_assigned_cert_to_client()
    {
        if ($response =  $this->requireRole(['5', '44'])) {
            return $response;
        }
        $cert_client_id  = $this->request->getPost('cert_client_id');
        // print_r($cert_client_id);
        // exit();
        $data_client = [
            'status' => 0,
            'last_updated_by' => session()->get('id_user'),
        ...
        $this->Certification_model->delete_assigned_cert_to_client($data_client, $cert_client_id);
    ...
    function Assign_certificate()
    {
        ...
        echo view('templates/header_view', $data);
        echo view('certification/certification_assign_lp', $data);
        echo view('templates/footer_view', $data);
    }
    ...
    function assign_cert_to_course()
    {
        ...
        return redirect()->to(base_url() . 'Certification/Certification_dashboard/Assign_certificate');
    }
    ...
    public function certification_view()
    {
        ...
        // print_r($data['getassignedclienttoeditable']);
        //    print_r($data['certificate_id']);
        // exit();
    ...
    function Assign_user_to_certification()
    {
        ...
        if (isset($_POST['certificate_id'])) {
            ...
            header('Location:' . base_url('Certification/Certification_dashboard/Assign_user_to_certification_view'));
            exit();
        }
        if (isset($_POST['type'])) {
            $data['type'] = $_POST['type'];
