...
class Emanual_client extends BaseController
{
    private $db;
    ...
    private function is_session_available()
    {
        ...
        if (!in_array('6', $arrayuserlevel) && !in_array('82', $arrayuserlevel)) {
    ...
    {
        if ($response =  $this->requireRole(['6', '82'])) {
            return $response;
        }
        $data = [];
        $userlevel = session()->get('userlevel');
        $arrayuserlevel  = explode(',', $userlevel);
        $data['header'] = 'e-Manual Clients';
        //$data['header_link'] = 'scorm_courses';
        //$data['sub_header_1'] = 'Create New Course';
        $data['form_link1'] = 'Aristo/aristo_client/view_client_course_assigned';
        $data['form_link2'] = 'Aristo/aristo_users';

        if (in_array('4', $arrayuserlevel) || in_array('6', $arrayuserlevel)) {
            $data['clientlist'] = $this->scorm_client_model->clientAristouserlist();
    ...
    public function view_client_course_assigned()
    {
        ...
        $data = [];
        helper(['form']);
        $data['header'] = 'Aristo Clients';
        $data['header_link'] = 'Aristo/aristo_client';
        $data['sub_header_1'] = 'Add Courses to Client';
        $data['form_link1'] = 'Aristo/aristo_client/delete_assigned_client_course';
        $data['form_link2'] = 'Aristo/aristo_client/add_course_to_client';
        $data['form_link3'] = 'Aristo/aristo_client/add_group_to_client';
        $data['form_link4'] = 'Aristo/aristo_client/usersassigned_report';
        $userlevel = session()->get('userlevel');
        $arrayuserlevel  = explode(',', $userlevel);
        if (in_array('4', $arrayuserlevel) || in_array('6', $arrayuserlevel)) {
            ...
            $data['all_courses'] = $this->scorm_course_model->getCoursesDetails(3);
            $data['coursegroupdata'] = $this->scorm_course_group_model->getCoursegroupdate(3);
            $data['getAllCoursesForClient'] = $this->scorm_client_model->getAllCoursesForClient($data['client_id'], 3);
            echo view('templates/header_view');
    ...
    public function usersassigned_report()
    {
        ...
        if (in_array('4', $arrayuserlevel) || in_array('6', $arrayuserlevel) || in_array('44', $arrayuserlevel)) {
            ...
            $data['header_link'] = 'Aristo/aristo_client/view_client_course_assigned';
            $data['header_sub_link'] = 'Aristo/aristo_client/usersassigned_report';
            $data['form_link'] = 'Aristo/aristo_client/userallcoursedetails';
    ...
    function userallcoursedetails()
    {
        ...
        if (in_array('4', $arrayuserlevel) || in_array('6', $arrayuserlevel) || in_array('44', $arrayuserlevel)) {
            ...
            $data['header_sub_link'] = 'Aristo/aristo_client/usersassigned_report';
            $data['form_link'] = 'Aristo/aristo_client/userallcoursedetails';
            $data['form_link1'] = 'Aristo/aristo_client/deleteScormdetails';
            $data['delete_enrollment'] = 'Aristo/aristo_client/delete_enrollment';
    ...
    public function delete_enrollment()
    {
        ...
        if ($result) {
            ...
            return redirect()->to(base_url() . '/Aristo/aristo_client/usersassigned_report');
        } else {
    ...
    //                     return redirect()->to(base_url() . '/aristo_client/view_client_course_assigned?client_id=' . $client_id);
    //                 } else {
    ...
    public function add_group_to_client()
    {
        ...
        if (in_array('4', $arrayuserlevel) || in_array('6', $arrayuserlevel)) {
            if ($this->request->getPost()) {
                ...
                if ($numcourses > 0) {
                    session()->setFlashdata('success', $numcourses . lang('Messages.Success_0023'));
                    return redirect()->to(base_url() . '/Aristo/aristo_client/view_client_course_assigned?client_id=' . $client_id);
                } else {
