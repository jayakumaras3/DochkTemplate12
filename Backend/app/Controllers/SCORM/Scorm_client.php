...
class Scorm_client extends BaseController
{
    private $db;
    ...
    private function is_session_available()
    {
        ...
        if (!in_array('6', $arrayuserlevel) && !in_array('44', $arrayuserlevel) && !in_array('45', $arrayuserlevel)) {
    ...
    public function index()
    {
        ...
        if ($response =  $this->requireRole(['6', '44', '45'])) {
            return $response;
        }
        $data = [];
        $data['header'] = 'Clients';
        ...
        $data['clientlist'] = $this->scorm_client_model->clientC4Uuserlist();
    ...
    public function reviews()
    {
        ...
        $data['my_reviews'] = $this->scorm_client_model->getmyreviews($user);
        $data['client_active_projects'] = $this->scorm_client_model->getClientProjects($client);
        echo view('templates/header_view');
        echo view('SCORM/scorm_client/sc_client_reviews', $data);
        echo view('templates/footer_view');
    }
    ...
    public function view_client_course_assigned()
    {
        ...
        $data['form_link1'] = 'SCORM/scorm_client/delete_assigned_client_course';
        $data['form_link2'] = 'SCORM/scorm_client/add_course_to_client';
        $data['form_link3'] = 'SCORM/scorm_client/add_group_to_client';
        $data['form_link4'] = 'SCORM/scorm_client/usersassigned_report';
        ...
        $data['coursegroupdata'] = $this->scorm_course_group_model->getCoursegroupdate(2);
    ...
    public function usersassigned_report()
    {
        ...
        $data['header_link'] = 'SCORM/scorm_client/view_client_course_assigned';
        $data['header_sub_link'] = 'SCORM/scorm_client/usersassigned_report';
        $data['form_link'] = 'SCORM/scorm_client/userallcoursedetails';
        $data['header_link_name'] = 'Add Courses to Client';
        $data['form_link1'] = 'SCORM/scorm_users/add_user_to_course';
    ...
    function userallcoursedetails()
    {
        ...
        $data['header_link_name'] = 'Add Courses to Client';
        $data['form_link1'] = 'SCORM/scorm_client/deleteScormdetails';
        $data['delete_enrollment'] = 'SCORM/scorm_client/delete_enrollment';
        $data['form_link2'] =  'SCORM/scorm_users/add_course_to_user';
    ...
    public function delete_enrollment()
    {
        ...
        if ($result) {
            ...
            return redirect()->to(base_url() . 'SCORM/scorm_client/usersassigned_report');
        } else {
    ...
    public function add_course_to_client()
    {
        ...
        $userlevel = session()->get('userlevel');
        $arrayuserlevel  = explode(',', $userlevel);
        if (in_array('44', $arrayuserlevel) || in_array('6', $arrayuserlevel)) {
            $newdata = [
    ...
    public function add_group_to_client()
    {
        ...
        if (in_array('44', $arrayuserlevel) || in_array('6', $arrayuserlevel)) {
            if ($this->request->getPost()) {
                ...
                if ($numcourses > 0) {
                    session()->setFlashdata('success', $numcourses . lang('Messages.Success_0042'));
                    return redirect()->to(base_url() . 'SCORM/scorm_client/view_client_course_assigned?client_id=' . $client_id);
                } else {
    ...
    public function add_bulk_course()
    {
        if ($response =  $this->requireRole(['6', '44', '45'])) {
            return $response;
        }
        $this->scorm_client_model->add_bulk_course();
    }
