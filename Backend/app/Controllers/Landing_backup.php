...
class Landing extends BaseController
{
    ...
    //     }
    //     $userpostCount = $this->post_model->getUserPostCount($user[0]['id_user']);
    ...
    //         'userpostCount' => isset($userpostCount[0]['status']) ? $userpostCount[0]['status'] : 0,
    ...
    public function login_register()
    {
        ...
        if (strtolower($contentType) !== $expected) {
        ...
        // Sanitize inputs
        $username = isset($json['username']) ? strip_tags(trim($json['username'])) : '';
        ...
        // Check validity
        if ($user['validity'] < $currdate && $user['validity'] != '0000-00-00' && $user['validity'] != '') {
            return $this->response->setJSON([
                'success' => false,
                'message' => 'Account validity expired. Please contact Admin.'
            ]);
        }
        ...
        $redirect = base_url($redirectMap[$userData['landing_page']] ?? 'my_training');
