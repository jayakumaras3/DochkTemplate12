...
class Profile extends BaseController
{
    ...
    {
        if ($response =  $this->requireRole(['5', '44', '3', '6'])) {
            return $response;
        }
    ...
    public function change_landing_page()
    {
        ...
        session()->setFlashdata('success', lang('Messages.Success_0008') . ' ' . lang('Messages.Success_0050'));
        return redirect()->to(base_url() . 'User_login/profile');
    }
    ...
    public function update_profile_data_personal()
    {
        ...
        return redirect()->to(base_url() . 'User_login/profile/update_data');
    }
    ...
    public function changeUserdata()
    {
        ...
        if ($this->request->getPost()) {
            ...
            if (!$this->validate($rules)) {
                ...
                // print_r($userdata[0]['session_version']);
                // exit();
                $newdata = [
                    'password' => trim(password_hash($this->request->getVar('password'), PASSWORD_DEFAULT)),
                    'session_version' => $userdata[0]['session_version'] + 1,
                    'last_updated_by' => session()->get('id_user'),
                    'last_updated_on' => time(),
                ];
                ...
                if ($result) {
                    ...
                    return redirect()->to(base_url() . 'ang/login?success=Password updated. Please login again.');
                } else {
    ...
    function delete_doc()
    {
        ...
        return redirect()->to(base_url() . 'User_login/profile/documents');
    }
            ...
            if (!$this->validate($rules)) {
                ...
                if ($file = $this->request->getFile('file')) {
                    if ($file->isValid() && !$file->hasMoved()) {
                        ...
                        if (file_exists(FCPATH . 'assets/assets/uploads/profile/' . $user . "/" . $randomname[0] . "/" . $imagename)) {
                            session()->setFlashdata('success', $imagename . ' ' . lang('Messages.Success_0051'));
                            return redirect()->to(base_url() . 'User_login/profile');
                        } else {
